...
DATASET_DIRECTORY = {
    ...
    "ne_medical_data":  "/workspace/Nepmedjp/pre-training/pre-trainingdataset/Nepalipubmeddata",  # FIX: was wrongly pointing to general nepali data
...
val_dataset = interleave_datasets(
    ...
    probabilities=probabilities,
    seed=42,
    stopping_strategy="first_exhausted",   # FIX: "all_exhausted" would cycle forever during eval
)
...
# ---- mT5 paper inverse square root learning rate schedule ----
# Formula: lr = 1 / sqrt(max(n, k))  where k = 10^4 (warmup steps)
# Setting base learning_rate=1.0 so LambdaLR output IS the actual LR.
# During warmup (n < k): lr = 1/sqrt(10000) = 0.01 (constant plateau)
# After warmup (n >= k): lr = 1/sqrt(n)          (decaying)

WARMUP_STEPS = 10_000  # k = 10^4 as specified in the mT5 paper

def inverse_sqrt_schedule(current_step: int) -> float:
    """
    Implements lr = 1 / sqrt(max(n, k)) from the mT5 paper.
    current_step is 0-indexed so we add 1 to avoid division by zero at step 0.
    """
    return 1.0 / math.sqrt(max(current_step + 1, WARMUP_STEPS))
...
training_args = TrainingArguments(
    ...
    learning_rate=1.0,              # base LR = 1.0; inverse_sqrt_schedule produces the actual LR
    warmup_steps=0,                 # warmup handled inside inverse_sqrt_schedule via the k clamp
    lr_scheduler_type="constant",   # disable HF built-in scheduler; we override below
    bf16=True,
    ...
    optim="adafactor",       # memory-efficient 8-bit AdamW optimizer from bitsandbytes # was "adamw_bnb_8bit"
...
# ---- Override HF scheduler with mT5 paper inverse square root schedule ----
# Must be done AFTER Trainer.__init__ which creates trainer.optimizer.
# LambdaLR multiplies base_lr (1.0) by the lambda output, so the lambda
# value is the actual learning rate at each step.
...
trainer.lr_scheduler = LambdaLR(
    trainer.optimizer,
    lr_lambda=inverse_sqrt_schedule,
)
print(
    f"\nLR schedule: inverse sqrt  |  k (warmup)={WARMUP_STEPS}"
    f"  |  LR at step 1 = {inverse_sqrt_schedule(0):.6f}"
    f"  |  LR at step {WARMUP_STEPS} = {inverse_sqrt_schedule(WARMUP_STEPS - 1):.6f}"
    f"  |  LR at step {total_steps} = {inverse_sqrt_schedule(total_steps - 1):.6f}"
)
